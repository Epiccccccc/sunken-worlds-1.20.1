...
public class HerringModel<T extends HerringModel> extends SinglePartEntityModel<T> {
	private final ModelPart Herring;
	private final ModelPart Body;

	public HerringModel(ModelPart root) {
		this.Herring = root.getChild("Herring");
		this.Body = root.getChild("Herring");
	}

	public static TexturedModelData getTexturedModelData() {
		ModelData modelData = new ModelData();
		ModelPartData modelPartData = modelData.getRoot();
		ModelPartData Herring = modelPartData.addChild("Herring", ModelPartBuilder.create(), ModelTransform.pivot(0.0F, 24.0F, 0.0F));
